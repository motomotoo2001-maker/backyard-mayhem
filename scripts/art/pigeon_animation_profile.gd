class_name PigeonAnimationProfile
extends RefCounted

const CANVAS_SIZE := Vector2i(256, 256)
const FLIGHT_ANCHOR := Vector2i(128, 156)
const ALLOW_HORIZONTAL_FLIP := true

const ACTIONS := {
    &"idle": {"frames": 4, "fps": 6.0, "loop": true},
    &"fly": {"frames": 8, "fps": 12.0, "loop": true},
    &"attack": {"frames": 6, "fps": 11.0, "loop": false, "events": {&"bomb_release": 3}},
    &"hurt": {"frames": 3, "fps": 12.0, "loop": false},
    &"death": {"frames": 6, "fps": 9.0, "loop": false},
}
