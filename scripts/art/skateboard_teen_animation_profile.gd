class_name SkateboardTeenAnimationProfile
extends RefCounted

const CANVAS_SIZE := Vector2i(288, 320)
const GROUND_ANCHOR := Vector2i(144, 300)
const ALLOW_HORIZONTAL_FLIP := true

const ACTIONS := {
    &"idle": {"frames": 4, "fps": 7.0, "loop": true},
    &"run": {"frames": 8, "fps": 15.0, "loop": true},
    &"attack": {"frames": 6, "fps": 14.0, "loop": false, "events": {&"hit": 3}},
    &"hurt": {"frames": 3, "fps": 13.0, "loop": false},
    &"death": {"frames": 7, "fps": 10.0, "loop": false},
}
