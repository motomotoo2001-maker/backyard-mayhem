class_name WaterVFXSpriteFramesValidator
extends RefCounted
...
static func validate(frames: SpriteFrames) -> Dictionary:
    ...
    for kind in Profile.kinds():
        ...
        var animation := StringName(kind)
        if not frames.has_animation(animation):
            missing.append(String(animation))
            continue

        var expected_count := Profile.frame_count(kind)
        var actual_count := frames.get_frame_count(animation)
        if actual_count != expected_count:
            bad_frame_counts.append(String(animation))

        if not is_equal_approx(frames.get_animation_speed(animation), Profile.fps(kind)):
            bad_speeds.append(String(animation))

        if frames.get_animation_loop(animation) != Profile.loops(kind):
